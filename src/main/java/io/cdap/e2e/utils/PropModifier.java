...
public class PropModifier implements EventListener {
  ...
  private void appendToProps(String fileName) {
    try {
      pluginPropBackup = (Properties) SeleniumHelper.connectProperties.clone();
      SeleniumHelper.connectProperties.load(PropModifier.class.getResourceAsStream("/" + fileName));
    } catch (IOException e) {
  ...
  private EventHandler<TestRunFinished> runFinishedHandler = new EventHandler<TestRunFinished>() {
    ...
    public void receive(TestRunFinished event) {
      SeleniumHelper.connectProperties = pluginPropBackup;
    }
  };
}
