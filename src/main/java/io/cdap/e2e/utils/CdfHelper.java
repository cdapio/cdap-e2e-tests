...
public interface CdfHelper {
  ...
  default int countOfNoOfRecordsTransferredToBigQueryIn(String records) throws IOException, InterruptedException {
    int countRecords;
    countRecords = GcpClient.countBqQuery(SeleniumHelper.readParameters(records));
    BeforeActions.scenario.write("**********No of Records Transferred in table" +
                                   SeleniumHelper.readParameters(records) + "*:" + countRecords);
